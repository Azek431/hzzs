...
> **当前实现状态（0.1.0）**：为简化首发，客户端**暂未启用 Ed25519 官方签名验签**。`.hzzsalg` 当前为 ZIP（`manifest.json` + `rules.json` + `CHANGELOG.txt`，**无 `signature.json`**）；客户端经 HTTPS + size/sha256 + ZIP 白名单校验后落盘。`release-index` 协议、目录 schema、发布流程保持原样，待签名私钥就位后重新启用签名分支即可回滚。下文以「⚠ 暂不启用」标注签名相关段落。
