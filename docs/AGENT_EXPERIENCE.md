...
- **FastContourV2 host smoke 与正式 host 分离**：`python tools/vision_v2/run_host_smoke.py` 只编 `vision_v2` 独立 exe（`build/vision-v2-host/`），**不要**塞进 `build_host.*` / `CMakeLists.txt` / APK。core/boundary/pipeline/profiles/sea_gap 烟雾；固定贴图 `generate_fixed_profiles_inc.py` → `fixed_profiles_v2.h`；海盐 `generate_sea_profiles_inc.py` → `sea_profiles_v2.h`（设计 1272×2772 + `scale_strip_profile`）；竹影缺口 `detect_bamboo_gaps` + `refine_bamboo_gap_contour`（局部暗区 Moore 边界，无 OpenCV）。**帧路径不读 JSON**。Shadow/JNI 另授权。
...
- **自动复活用控件树而非找色**：快手小游戏「原地复活 / 重新冒险」TextView 常不可点且 dump 无按钮级 resource-id；`findAccessibilityNodeInfosByText` 精确文案 → 最近 clickable 祖先 → `boundsInScreen` 中心归一化点击。`autoReviveEnabled` 与障碍自动操作独立，默认开，冷却约 0.3s；诊断串 `plan revive press@x,y` / `skip:revive_no_a11y`。
- **Shizuku DOUBLE_JUMP 超时与排查日志**：`GestureArbiter` 超时预算须覆盖两按 + 间隔 + 冷启动多条 input 候选；超时 detail 含 `budget/presses/waited`。Shell 回执带 `via=/tries=`。决策 ring（`AlgorithmRuntimeTrace` 最近 64 条 + 诊断「Algorithm decisions」）含 `calc` 触发距离/gap 与 `dispatch_*`；`package_gate` 会打印 restrict/allow 列表。排障：`logLevel=DEBUG` 看 `algo.frame/det`，INFO 已能看 decision 时间线。
...
- **算法改内容自动 PATCH**：不要只改 `rules.json` 不升版本（远端同 version 哈希不可变）。用 `prepare_algorithm_release.py --auto-bump` 或 push `algorithm-packs/**` 让 CI 对比远端后 PATCH+发布；大改用 `--level minor/major`。
