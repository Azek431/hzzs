...
2. 独立 C++ core 与 host test，不加入现有 native library；
   **当前进度**：`python tools/vision_v2/run_host_smoke.py`（core + boundary + pipeline + profiles；可选 ASan/UBSan）。
   已有固定容量条带扫描/稀疏验证/轮廓放置 API（`fast_contour_pipeline.*`）。
   甜品/竹影固定贴图 profile：`fixed_profiles_v2.json` → `generate_fixed_profiles_inc.py` → `generated/fixed_profiles_v2.h`（编译期注入，帧路径不读 JSON）。
   海盐 scan-point 与竹影缺口仍待迁。
