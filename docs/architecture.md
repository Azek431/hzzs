...
仅监听 `127.0.0.1`，每次启动生成随机 Bearer（恒时比较），Origin 非空时须为本机回环。传输为 Streamable HTTP（`POST /mcp`），协议版本协商 `2025-06-18`（兼容 `2025-03-26` / `2024-11-05`）。
...
权限：只读、每次确认、会话信任（绑定已 initialized 的内存会话，不持久化）、完整访问。`tools/call` 须完成握手；`tools/list` 可无会话探测。API 34+ 前台服务须带 `SPECIAL_USE` type。`preview_settings`/`save_settings` 相对已保存 baseline 做权限型字段收敛。GET `/mcp` 返回 405（不提供 SSE 推送流）。
