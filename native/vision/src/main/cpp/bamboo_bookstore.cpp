...
namespace hzzs {
Result analyze_bamboo(const FrameView& f, int work_width) {
    Result out;
    ...
    if(!player_found || blocked) return out;
    ...
    for(const auto& c:statues){
        ...
        if(w<.05f||w>.34f||h<.075f||h>.35f||bottom_gap>.13f||c.right<=player_component.right) continue;
