            ...
            item {
                ElevatedCard {
                    Column(Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        Text("捕获：${if (status.captureReady) "已就绪" else "等待授权或连接"} · ${status.activeBackend.name}")
                        Text("${"%.1f".format(status.fps)} FPS · ${"%.2f".format(status.processingMs)} ms · ${status.obstacleCount} 个障碍")
