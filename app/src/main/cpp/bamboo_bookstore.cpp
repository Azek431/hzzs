...
namespace hzzs {
Result analyze_bamboo(const FrameView& f, int work_width, int enabled_kind_mask, bool detect_player,
                      float fixed_player_x_ratio, const SceneAlgorithmParamsNative& params) {
    Result out;
    const auto ground = estimate_bamboo_ground(f, params);
    ...
    if (kind_enabled(enabled_kind_mask, Kind::HANGING_BRUSH)) {
        auto brushes = column_regions(
            f,
            [&](int r, int g, int b) {
                return r < colors.brush_dark_max && g < colors.brush_dark_max - 10 &&
                       b < colors.brush_dark_max - 20;
            },
            static_cast<int>(f.height * .18f),
            ground.y + static_cast<int>(f.height * .04f),
            x_stride,
            std::max(4, static_cast<int>(f.height * .04f) / x_stride),
        ...
        for (const auto& c : brushes) {
            const float w = normalized_width(c, f), h = normalized_height(c, f),
                        aspect = h / std::max(w, .001f);
            if (w < params.brush_width_min || w > params.brush_width_max ||
                h < params.brush_height_min || h > params.brush_height_max || aspect < 1.65f ||
                center_y(c, f) < .28f) {
                continue;
            }
            Rect box = norm(c, f);
            box.left = std::max(0.0f, box.left - .040f);
            box.right = std::min(1.0f, box.right + .040f);
            box.top = std::max(0.0f, box.top - .025f);
    ...
    if (kind_enabled(enabled_kind_mask, Kind::PANDA_STATUE)) {
        auto statue_parts = components(
            f,
            [&](int r, int g, int b, int, int y) {
                const int maxc = std::max({r, g, b}), minc = std::min({r, g, b});
                return y > f.height * .34f && y < ground.y + f.height * .06f &&
                       maxc - minc < colors.statue_chroma_max && r > 32 && r < 185 && g < 190 &&
                       b < 195;
            },
            x_stride,
            std::max(5, 14 / x_stride));
        auto statues = merge_components(
            std::move(statue_parts),
        ...
        for (const auto& c : statues) {
            ...
            if (w < params.statue_width_min || w > params.statue_width_max ||
                h < params.statue_height_min || h > params.statue_height_max || bottom_gap > .13f ||
                c.right <= player_right) {
                continue;
            }
            const bool overlaps_brush = std::any_of(
                brush_regions.begin(),
                brush_regions.end(),
                [&](const Rect& brush) { return intersection_over_candidate(c, brush, f) > .35f; });
            if (overlaps_brush) continue;
            const float neutral = region_fraction(
                f, c, [](int r, int g, int b) {
                    return std::max({r, g, b}) - std::min({r, g, b}) < 52 && r < 195;
                });
            const float dark = region_fraction(
    ...
    if (kind_enabled(enabled_kind_mask, Kind::BAMBOO_GAP)) {
        auto gaps = column_regions(
            f,
            [](int r, int g, int b) {
                return r > 35 && r > g * 1.10f && g > b * 1.05f && r < 160 && b < 100;
            },
            ground.y - static_cast<int>(f.height * .03f),
            static_cast<int>(f.height * .97f),
            x_stride,
            std::max(6, static_cast<int>(f.height * .09f) / x_stride),
            static_cast<int>(f.width * .135f),
            static_cast<int>(f.width * .02f));
        for (const auto& c : gaps) {
            const float w = normalized_width(c, f), h = normalized_height(c, f);
            if (w < params.gap_width_min || w > params.gap_width_max || h < params.gap_height_min ||
                c.top > ground.y + f.height * .065f) {
                continue;
            }
            const float continuous_ground = horizontal_fraction(
                ...
                [&](int r, int g, int b) { return bamboo_green(r, g, b, colors); });
