// 火崽崽助手（HZZS）角色状态机 — 实现层。
//
// 根据连续帧的玩家矩形边界推断运动姿态：奔跑、起跳上升、滞空顶点、下落、滑铲、飞行。
//
// 核心机制：
// 1. 基线建立：第一帧有效数据记录角色底部位置和高度作为基准
// 2. 速度计算：基于帧间时间差和底部位置变化率得出垂直速度
// 3. 着地判定：当前底部与基线底部的差距 <= kAirborneThreshold → grounded
// 4. 姿态判定：根据垂直速度和基线高度比例区分奔跑/滑铲/起跳/下落/顶点
// 5. 基线平滑：指数移动平均（系数 0.08）防止单帧跳动导致基线漂移
